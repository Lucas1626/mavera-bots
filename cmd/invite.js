...
exports.run = async (client, message, args) => {
    ...
    if (args[0] == "genelekle") {
      message.guild.fetchInvites().then(async Invites => {
        ...
        }, index*2500)})}).catch(console.error)
    ...
    if (args[0]) {
        ...
        .setFooter(ayar.footer)
        .setTimestamp()
        return message.channel.send(embedvictim)} else { return message.channel.send(embed)}}

exports.conf = {enabled: true, guildOnly: true, aliases: ['invites', 'davetim', 'davetlerim', 'rank'], permLevel: 0}
